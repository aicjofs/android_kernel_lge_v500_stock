...
wf_chspec_valid(chanspec_t chanspec)
{
	...
	if (CHSPEC_IS2G(chanspec)) {
		...
		if (chspec_bw == WL_CHANSPEC_BW_8080) {
			uint16 ch1, ch2;
#ifdef CUSTOMER_HW10
			uint chan1 = CHSPEC_CHAN1(chanspec) >> WL_CHANSPEC_CHAN1_SHIFT;
			uint chan2 = CHSPEC_CHAN2(chanspec) >> WL_CHANSPEC_CHAN2_SHIFT;
			ch1 = (chan1 < WF_NUM_5G_80M_CHANS) ? wf_5g_80m_chans[chan1] : 0;
			ch2 = (chan2 < WF_NUM_5G_80M_CHANS) ? wf_5g_80m_chans[chan2] : 0;
#else
