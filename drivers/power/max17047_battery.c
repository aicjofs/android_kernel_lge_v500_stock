...
int max17047_get_capacity_percent(void)
{
	...
	battery_soc = (battery_soc  /9224) + 5 ; //          
	battery_soc /= 10 ; 
...
static int max17047_new_custom_model_write(void)
{
	...
	max17047_write_reg(max17047_i2c_client, MAX17047_I_CHG_TERM, 0x03C0);//                         
...
//                                          
	{
		...
		if ( ret == 2)
			...
			max17047_force_custom_model_write(); //                                
		else if ( ret == 0)
...
static int max17047_access_control_of_flash(void)
{
	...
	if (access_nand_setting_flag == 2)
		ret = max17047_save_bat_info_check(); //                                                     
	else if (access_nand_setting_flag == 1)
		ret = max17047_restore_bat_info_from_flash(); //                                                                       
	...
	if (ret == 0){
		...
		if ( ret == 2)
			access_nand_setting_flag = 2; //                                                     
		else if ( ret == 0)
			access_nand_setting_flag = 1;//                                                                       
...
static ssize_t at_fuel_guage_reset_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int r = 0;
	bool ret = 0;
	ret = max17047_set_battery_atcmd(0, 100);  //                        
	if ( ret == 1)
			...
			const struct i2c_device_id *id)
{
	...
	if ( ret == 2)
		...
		max17047_new_custom_model_write(); //                                
	else if ( ret == 0)
		printk("POWER_ON reset. Custom Model Success. \n");//                                                                       
