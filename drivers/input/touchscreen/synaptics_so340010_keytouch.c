...
struct so340010_device {
	...
	struct work_struct	work_getkey;			/*                            */
	struct delayed_work	work_report;			/*                                  */
	struct delayed_work	work_lock;				/*                            */

	struct kfifo			key_fifo;				/*                   */
	spinlock_t			key_fifo_lock;			/*               */

	struct TSKEY		key_home;				/*          */
	struct TSKEY		key_menu;				/*          */
	struct TSKEY		key_back;				/*          */

	bool					touchkey_lock;			/*              */
	bool					touchkey_btn_press;	/*                           */
...
static void so340010_i2c_work_init(struct work_struct *work)
{
	...
	if (unlikely(pDev->initialized == false)) {
		if (likely(pDev->gpio_attn)) {
ATTN_CHECK :
			if (gpio_get_value(pDev->gpio_attn) == ATTN_HIGH) { //                
				mdelay(10);
				if (retry_cnt++ < ERROR_RETRY_COUNT) {
