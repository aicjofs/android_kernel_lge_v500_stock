...
static struct msm_camera_i2c_reg_conf ce1702_prev_settings[] = {
	{0x54,	0x1C01, MSM_CAMERA_I2C_WORD_DATA},	//               
	{0x11,		0x00},	  //             
};
...
/*                                                                         */
		...
		if(ce1702_focus_mode == AF_MODE_AUTO) {
			wdata[0] = 0x00;
			rc = ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x35, wdata,  1);	//         
			if (rc < 0)
...
int8_t ce1702_start_af(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (ce1702_is_doing_touchaf) {
		...
		if ((ce1702_focus_mode == AF_MODE_CAF_PICTURE) || (ce1702_focus_mode == AF_MODE_CAF_VIDEO)) {
			...
			/*                                                                   */
			data = 0x05; //         
			rc = ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x42, &data, 1); //                       
			cnt = 0;
			do {
...
int8_t ce1702_stop_af(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x35, NULL, 0); //     
	cnt = 0;
	do {
	...
	if (rdata[0] == 1) {
		data[0] = 0x00;
		ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x2C, data, 1); //          
		do {
...
int8_t ce1702_set_window(struct msm_sensor_ctrl_t *s_ctrl, int16_t *window, int16_t sw)
{
	...
	if(ce1702_focus_mode != AF_MODE_CAF_VIDEO) {
		...
		data[2] = window[0] & 0xFF; //        
		data[3] = window[0] >>0x08; //        
		data[4] = window[1] & 0xFF; //        
		data[5] = window[1] >>0x08; //        
		data[6] = window[2] & 0xFF; //        
		data[7] = window[2] >>0x08; //        
		data[8] = window[3] & 0xFF; //        
		data[9] = window[3] >>0x08; //        
		rc = ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x41, data, 10); //               
		if (rc < 0)
			LDBGE("%s: ce1702_i2c_write failed(1) \n", __func__);
...
int8_t ce1702_set_wb_setting(struct msm_sensor_ctrl_t *s_ctrl, uint8_t wb)
{
	...
	rc = ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x1A, &wb_mode, 1);	//               
	if (rc < 0)
		LDBGE("%s: ce1702_i2c_write failed(1) \n", __func__);
	rc = ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x04, data, 2);	//      
	if (rc < 0)
...
int8_t ce1702_set_zoom_ratio(struct msm_sensor_ctrl_t *s_ctrl, int32_t zoom)
{
	...
	data = zoom;
	rc = ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0xB9, &data, 1);	//           
	if(rc < 0) {
...
int8_t ce1702_set_manual_focus_length(struct msm_sensor_ctrl_t *s_ctrl, int32_t focus_val)
{
	...
	data[0] = 0x01;	//                 
	data[1] = focus_val & 0xFF; //        
	data[2] = focus_val >> 0x08; //        
	mdelay(5);
...
int32_t ce1702_object_tracking(struct msm_sensor_ctrl_t *s_ctrl, struct rec_t * rect_info)
{
	...
	if(rect_info->mode) //  
	{
		...
		wdata[0] = 0x04;
		wdata[1] = 0x00;
		wdata[2] = center_x&0xFF;  //     
		wdata[3] = center_x>>0x8; //     
		wdata[4] = center_y&0xFF; //     
		wdata[5] = center_y>>0x8; //     
		wdata[6] = 0x00;
		wdata[7] = 0x00;
		wdata[8] = 0x00;
		wdata[9] = 0x00;
		rc = ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x41, wdata,  10);	//                        
		if (rc < 0)
			LDBGE("%s: ce1702_i2c_write failed(1) \n", __func__);
		wdata[0] = 0x04;
		rc = ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x42, wdata,  1);	//                  
	...
	{
		wdata[0] = 0x00;
		ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x42, wdata,  1);	//                   
		if (rc < 0)
			LDBGE("%s: ce1702_i2c_write failed(3) \n", __func__);
	}
...
int8_t ce1702_set_exif_gps(struct msm_sensor_ctrl_t *s_ctrl, struct k_exif_gps_t *gps_info)
{
	...
	//              
	data[0] = 0x20;
	data[1] = (gps_info->gpsDateStamp[0]) & 0xFF; //        
	data[2] = (gps_info->gpsDateStamp[0]) >>0x08; //        
...
int8_t ce1702_set_manual_VCM_position(struct msm_sensor_ctrl_t *s_ctrl, int32_t focus_val)
{
	...
	LDBGI("%s: manual focus value=%d\n", __func__, focus_val);
		...
		data[0] = 0x01;	//                  
		data[1] = (unsigned char)(focus_val & 0xFF); //        
		data[2] = (unsigned char)(focus_val >>0x08); //        
		LDBGI("%s: CMD=0x%02X, 0x%02X, 0x%02X, 0x%02X\n", __func__, 0x33, data[0], data[1], data[2]);
		rc = ce1702_i2c_write(ce1702_s_interface_ctrl->sensor_i2c_addr, 0x33, data, 3);
		if (rc < 0)
			LDBGE("%s: ce1702_i2c_write failed(1) \n", __func__);
...
static struct msm_sensor_fn_t ce1702_func_tbl = {
	.sensor_start_stream = ce1702_sensor_start_stream, //                        
	.sensor_stop_stream = ce1702_sensor_stop_stream,//                       
	...
	.sensor_start_af = ce1702_start_af,									//                                  
	.sensor_stop_af = ce1702_stop_af,									//                                 
	.sensor_set_af_window = ce1702_set_af_window,						//                                      
	...
	.sensor_set_led_flash_mode = ce1702_sensor_set_led_flash_mode,		//                                               
	.sensor_set_antibanding_ce1702 = ce1702_sensor_set_antibanding,		//                                        
	.sensor_set_ae_window = ce1702_set_ae_window,						//                                      
	.sensor_object_tracking = ce1702_object_tracking,						//                                                                             
	.sensor_set_aec_awb_lock = ce1702_set_aec_awb_lock,					//                                             
	.sensor_dim_info = ce1702_dim_info,									//                                                                              
	.sensor_get_cam_open_mode = ce1702_get_cam_open_mode,			//                                                 
	.sensor_set_iso = ce1702_set_iso,									//                                               
	.sensor_set_manual_scene_mode = ce1702_set_manual_scene_mode,		//                                                   
	.sensor_set_gyro_data = ce1702_set_gyro_data,						//                                    
	.sensor_set_wdr = ce1702_set_WDR,									//                                        
	.sensor_set_exif_rotation = ce1702_set_exif_rotation,					//                                                         
	.sensor_set_exif_gps = ce1702_set_exif_gps,							//                                                 
	.sensor_set_asd_enable = ce1702_asd_enable,							//                                       
