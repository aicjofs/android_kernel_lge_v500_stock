...
int i;
	...
	if(host->index==1)
	{
		for (i = 0; i <l_sdcc_host->plat->pin_data->pad_data->drv->size; i++) {
		
			switch(l_sdcc_host->plat->pin_data->pad_data->drv->on[i].val)
			{
				case GPIO_CFG_2MA:
					str = "2MA";
					break;
		...
		if(l_sdcc_host->plat->pin_data->pad_data->drv->on[i].no == TLMM_HDRV_SDC3_CLK)
		{	
			seq_printf(s, "Drive strength CLK:\t %s \n",str );
		}

		else if(l_sdcc_host->plat->pin_data->pad_data->drv->on[i].no==TLMM_HDRV_SDC3_CMD)
		{	
			
			seq_printf(s, "Drive strength CMD:\t %s \n",str );
		}

		else if(l_sdcc_host->plat->pin_data->pad_data->drv->on[i].no == TLMM_HDRV_SDC3_DATA)
		{	
			seq_printf(s, "Drive strength DATA:\t %s \n",str );
		}
	}
	}
#endif
