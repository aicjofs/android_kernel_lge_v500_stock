  ...
  int rtncode = 0;
   ...
   if (chipid0_rf == 0x03) {    //           
	...
	int indx;
		...
		{0x3B, 0xD1}, {0x3C, 0xA1},
		{0x3B, 0xD7}, {0x3C, 0x00},
		{0x28, 0x20}, {0x29, 0x56}, {0x2A, 0x24}, {0x2B, 0x4A}, //               
		{0xD0, 0x02},  //               
		{0x3B, 0xC3}, {0x3C, 0x40},  //     
		{0x28, 0xEF}, {0x29, 0x08}, {0x2A, 0xC0}, {0x2B, 0x04}, //     
		{0x4F, 0x03},  //     
		{0x3B, 0xEA}, {0x3C, 0x99},  //     
		{0x3B, 0x5D}, {0x3C, 0x40},  //     
		{0x40, 0x4D}, {0x41, 0x22},  //     
...
void mb86a35s_irq_work(struct work_struct *work)
{
	...
	if (pm_qos_request_active(&mmb_pm_qos_list)){
		 pm_qos_update_request(&mmb_pm_qos_list, 20 /*         */);
	}
#endif
	//                  
	...
	//      
		...
		else
		{
			...
			{
				ret = mbt_dataring_write(buf_id, _bufp +3, read_size); //                      
				if(ret < 0)
				{
					mbt_dataring_flush(buf_id);
					ret = mbt_dataring_write(buf_id, _bufp +3, read_size); //           
