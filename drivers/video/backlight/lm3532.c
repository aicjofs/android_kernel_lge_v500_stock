...
static void lm3532_set_main_current_level(struct i2c_client *client, int level)
{
	...
	if (level != 0) {
		...
		lm3532_write_reg(client, 0x70, cal_value); //                                             
	} else {
		lm3532_write_reg(main_lm3532_dev->client, 0x1d, 0x00);	/*               */
	}
...
void lm3532_backlight_on(int level)
{
	...
	if (backlight_status == BL_OFF) {
		...
		lm3532_write_reg(main_lm3532_dev->client, 0x1d, 0x01);	/*              */
#if defined(CONFIG_LGE_BACKLIGHT_CABC)
		lm3532_write_reg(main_lm3532_dev->client, 0x13, 0x06);	/*                                                                                               */
#else
		lm3532_write_reg(main_lm3532_dev->client, 0x16, 0x03);	/*                                                               */
#endif //           
		lm3532_write_reg(main_lm3532_dev->client, 0x17, 0x10);  /*                                       */
	}
	mdelay(1);   /*              */
