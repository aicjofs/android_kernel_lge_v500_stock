...
static struct early_suspend early_suspend;
	...
	defined(CONFIG_FB_MSM_MIPI_LGIT_VIDEO_FHD_PT) || \
...
static void lm3533_set_main_current_level(struct i2c_client *client, int level)
{
	...
	if (level != 0) {
		...
		lm3533_write_reg(main_lm3533_dev->client, 0x03, cal_value);
	//                                                      
	} else {
...
void lm3533_backlight_on(int level)
{
	...
	printk(" ### %s ### \n",__func__);
		...
		lm3533_write_reg(main_lm3533_dev->client, 0x02, 0x30); /*                                               */

		lm3533_write_reg(main_lm3533_dev->client, 0x01, 0x08); 	/*                                           */

		lm3533_write_reg(main_lm3533_dev->client, 0x03, 0xFF);  /*                                       */

		lm3533_write_reg(main_lm3533_dev->client, 0x05, 0x15);  /*                                        */

		lm3533_write_reg(main_lm3533_dev->client, 0x00, 0x15);	/*                                                      */
...
void lm3533_lcd_backlight_set_level(int level)
{
	...
	if (is_early_suspended) {
		requested_in_early_suspend_lcd_level = level;
		printk("[joon9] already backlight off by early suspend\n ");
		return;
	}
